const employeeModel = require('./../../config/db.config');
...
Employee.create = function(newEmployee, result) {
   employeeModel.query("INSERT INTO employees SET ?", newEmployee, function(err, res) {
      if(err) {
...
Employee.findById = function(id, result) {
   employeeModel.query("SELECT * FROM employees WHERE id = ? ", id, function (err, res) {
      if(err) {
        console.log("error: ", err);
        result(err, null);
      } else {
...
Employee.findAll = function (result) {
   employeeModel.query("SELECT * FROM employees", function (err, res) {
      if(err) {
      console.log("error: ", err);
      result(null, err);
      } else {
